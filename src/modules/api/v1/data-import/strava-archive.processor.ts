...
@Injectable()
export class StravaArchiveProcessor {
  private readonly logger = new Logger(StravaArchiveProcessor.name);
  ...
  async processArchive(zipBuffer: Buffer, job: DataImportJob): Promise<ArchiveProcessingResult> {
    ...
    try {
      ...
      // Find all activity files (FIT, GPX, TCX) in the activities folder
      const activityFiles = zipEntries.filter((entry) => {
        const entryPath = entry.entryName.toLowerCase();
        return (
          !entry.isDirectory &&
          (entryPath.includes('activities/') || entryPath.includes('export_')) &&
      ...
      this.logger.log(`Found ${result.totalFiles} activity files in Strava archive`);
      ...
      for (const entry of activityFiles) {
        try {
          ...
          // Try to extract activity ID from filename (Strava pattern: {id}.fit or {id}.fit.gz)
          const externalId = this.extractStravaActivityId(fileName);
          ...
          const execution = await this.workoutExecutionRepo.create({
            ...
            external_id: externalId ?? `strava_archive:${fileName}`,
            notes: `Imported from Strava archive: ${fileName}`,
          });
          ...
          const sampledMetrics = this.sampleMetrics(parsed.metrics, 100); // Keep ~100 points per metric type
      ...
      this.logger.error(`Failed to process Strava archive: ${error}`);
      throw error;
    }
  ...
  private extractStravaActivityId(fileName: string): string | null {
    // Strava export filenames are typically: {activity_id}.fit.gz or {activity_id}.gpx
